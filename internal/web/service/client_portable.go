...
// ExportForInbounds returns clients attached to at least one of the given
// inbound ids, with only those attachments included in each item's inboundIds.
// An empty allowed list yields an empty export.
func (s *ClientService) ExportForInbounds(inboundIds []int) ([]ClientCreatePayload, error) {
	if len(inboundIds) == 0 {
		return []ClientCreatePayload{}, nil
	}
	all, err := s.ExportAll()
	if err != nil {
		return nil, err
	...
	out := make([]ClientCreatePayload, 0)
	for _, item := range all {
		filtered := make([]int, 0, len(item.InboundIds))
		...
		item.InboundIds = filtered
		out = append(out, item)
	}
	return out, nil
}

// SanitizeImportForInbounds keeps only inbound attachments within allowed and
// drops items that would end up with no inbound. Reseller restore must not
// recreate orphan client records.
func SanitizeImportForInbounds(items []ClientCreatePayload, allowedInboundIds []int) []ClientCreatePayload {
	...
	out := make([]ClientCreatePayload, 0, len(items))
