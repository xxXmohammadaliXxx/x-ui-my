...
func (j *XrayTrafficJob) Run() {
	...
	if clientsDisabled {
		...
		if restartOnDisable || settingErr != nil {
			// Default / safe path: restart immediately so the rebuilt config
			// (without the just-disabled clients) drops their stale sessions.
			if err := j.xrayService.RestartXray(true); err != nil {
				logger.Warning("restart xray after disabling clients failed:", err)
				j.xrayService.SetToNeedRestart()
			}
		} else {
			// restartOnDisable explicitly false: don't restart immediately, but
			// still schedule a deferred restart so over-quota / expired clients'
			// already-established sessions are dropped on the next cron tick
			// instead of proxying indefinitely until a manual restart. xray-core
			// has no per-user connection-kill API, so a config-rebuild restart is
			// the only reliable way to disconnect them.
			j.xrayService.SetToNeedRestart()
		}
